...
def execute_command(command, arguments, output):
    ...
    if result.returncode == 0:
        ...
        print(f"Error old_code_2: {result.returncode}")
        return None


## Theoretically working
def start_dool(out):
    subprocess.Popen(["bash", "old_code_2/analysis/dool/start_dool.bash", out])
...
def stop_dool():
    # Baking in 10 seconds to monitor the system without the process
    time.sleep(10)
    subprocess.run(["bash", "old_code_2/analysis/dool/kill_dool.bash"])
...
def get_args(tool, assembler, dataset):
    ...
    if tool == "assembly" or \
            tool in [os.path.splitext(f)[0] for f in os.listdir(f"old_code_2/{path_assembler}") if
                     os.path.isfile(os.path.join(f"old_code_2/{path_assembler}", f))]:

        args_path = file_dict.get(f"asm-args_{assembler}_{dataset}", "")

    elif tool == "retrieval" or \
            tool in [os.path.splitext(f)[0] for f in os.listdir(f"old_code_2/{path_retriever}") if
                     os.path.isfile(os.path.join(f"old_code_2/{path_retriever}", f))]:
...
def main():
    test_path_exec = "./old_code_2/assembly/path_assembler/echo.txt"
